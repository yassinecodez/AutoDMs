  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0A0A0A] border border-[#222222] rounded-xl p-3 flex flex-wrap gap-3 items-center justify-between">
        ...
        <div className="flex items-center bg-[#111111] border border-[#262626] rounded-lg p-0.5 text-xs font-medium text-zinc-400 overflow-x-auto max-w-full">
          <button
            onClick={() => setSelectedCategory("ALL")}
            className={`px-3 py-1.5 rounded-md transition-colors whitespace-nowrap ${
              selectedCategory === "ALL" ? "bg-[#222222] text-white" : "hover:text-zinc-200"
            }`}
          >
            All Templates
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-md transition-colors whitespace-nowrap ${
              selectedCategory === "SELL_EARN" ? "bg-[#222222] text-white" : "hover:text-zinc-200"
            }`}
          >
            Sell & Earn
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-md transition-colors whitespace-nowrap ${
              selectedCategory === "CAPTURE_LEADS" ? "bg-[#222222] text-white" : "hover:text-zinc-200"
            }`}
          >
            Capture Leads
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-md transition-colors whitespace-nowrap ${
              selectedCategory === "AUDIENCE_ENGAGEMENT" ? "bg-[#222222] text-white" : "hover:text-zinc-200"
            }`}
          >
            Audience Engagement
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredTemplates.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="bg-[#0A0A0A] border border-[#222222] hover:border-zinc-700 rounded-2xl p-6 flex flex-col justify-between space-y-5 transition-all shadow-sm group"
            >
              <div className="space-y-4">
                ...
                <div className="p-3.5 bg-[#111111] border border-[#222222] rounded-xl space-y-2 text-xs">
                  {/* Trigger Pill */}
                  <div className="flex items-center gap-1.5 text-zinc-400 text-[11px]">
                    <span className="text-zinc-500 font-medium">When:</span>
                    {item.triggerSource === "STORY_MENTIONS" ? (
                      <span className="text-zinc-200 font-medium bg-[#181818] px-2 py-0.5 rounded border border-[#262626]">
                        Story Mention
                      </span>
                    ) : (
                      <span className="text-white font-mono bg-[#181818] px-2 py-0.5 rounded border border-[#262626]">
                        "{item.triggerKeyword}"
                      </span>
                    )}
                  </div>
                  ...
                  {/* Message Snippet */}
                  <div className="space-y-1">
                    <span className="text-zinc-500 font-medium text-[10px] block">DM Response:</span>
                    <p className="text-[11px] text-zinc-300 font-mono line-clamp-2 bg-[#0A0A0A] p-2 rounded border border-[#222222]">
                      "{item.replyMessage}"
                    </p>
                  </div>
                  ...
                  {/* Button Mockup */}
                  {item.buttonTitle && (
                    <div className="flex items-center justify-center p-2 rounded-lg bg-[#181818] border border-[#262626] text-white text-[11px] font-medium gap-1.5 mt-1">
              ...
              <div className="pt-2">
                <Link
                  ...
                  className="w-full h-10 rounded-xl bg-white hover:bg-zinc-200 text-black font-medium text-xs inline-flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                >
